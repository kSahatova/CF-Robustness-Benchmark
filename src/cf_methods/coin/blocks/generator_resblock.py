...
from .cbn import ConditionalBatchNorm2d
...
class GeneratorResBlock(nn.Module):
    ...
    def __init__(
        ...
        ds_conv = SNConv2d if use_snconv else nn.Conv2d
        self.left_branch = nn.Sequential(
            upsample,
            ds_conv(in_channels, out_channels, kernel_size=1, padding=0, bias=False),
        )

        self.cbn_relu_first = ConditionalBatchNorm2d(in_channels, num_classes, act=nn.ReLU())
        self.cbn_relu_second = ConditionalBatchNorm2d(out_channels, num_classes, act=nn.ReLU())
    ...
    def initialize(self):
        ...
        for m in self.left_branch.modules():
            if isinstance(m, (SNConv2d, nn.Conv2d)):
