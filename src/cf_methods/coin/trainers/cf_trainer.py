...
from src.datasets import MedMNISTDataset
from src.datasets import AUGMENTATIONS
from torch.utils.data import DataLoader

from src.recourse_methods.coin.model import CounterfactualCGAN
from src.recourse_methods.coin.utils import AvgMeter, save_model
from src.recourse_methods.coin.trainers import BaseTrainer
...
class CounterfactualTrainer(BaseTrainer):
    ...
    #         dataset = MedMNISTDataset(self.opt.dataset.data_flag, 
    #                                   split=ds_name, 
    #                                   data_dir=self.opt.dataset.data_dir, 
    #                                   size=self.opt.dataset.img_size, 
    #                                   transform=transforms, 
    #                                   download=self.opt.dataset.download,
    #                                   increase_channels=False,
    #                                   undersample=undersample_flag)
    #         datasets[ds_name] = dataset
    #         loaders.append(DataLoader(dataset=dataset, batch_size=self.opt.dataset.batch_size, shuffle=True))

    #     return loaders  # (loaders, sampler_labels) if ret_cf_labels else
