...
class ResBlocksDiscriminator(nn.Module):
    ...
    def __init__(
        ...
        self.blocks = nn.ModuleList(
            [
                DiscriminatorResBlock(in_channels, out_channels[0], downsample_scales[0], first_block=True),
                *( 
                    DiscriminatorResBlock(out_channels[i-1], out_channels[i], downsample_scales[i]) 
...
if __name__ == '__main__':
    disc = ResBlocksDiscriminator(n_classes=2)
