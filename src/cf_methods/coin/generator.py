...
class ResBlocksEncoder(nn.Module):
    ...
    def __init__(self, in_channels=1, downsample_scales=[2, 2, 2, 2, 2], out_channels=[64, 128, 256, 512, 1024], use_snconv=True):
        ...
        self.blocks = nn.ModuleList(
            [
                encoder_resblock.EncoderResBlock(64, out_channels[0], 
                                                 downsample_scales[0],
                                                 use_snconv=use_snconv),
                *(
                    encoder_resblock.EncoderResBlock(out_channels[i-1], 
                                                     out_channels[i], 
                                                     downsample_scales[i], 
                                                     use_snconv=use_snconv)
...
class ResBlocksGenerator(nn.Module):
    ...
    def __init__(
        ...
        if skip_conn is None:
            ...
            in_channels = [in_channels[0]] + [out_channels[i-1] + (in_channels[i] 
                                                                   if i in self.skip_conn else 0) 
                                              for i in range(1, self.n_blocks)]
        ...
        self.blocks = nn.ModuleList(
            [
                gen_resblock.GeneratorResBlock(n_classes, in_channels[0], out_channels[0], 
                                         scale_factor=upsample_scales[0], upsample_kind=upsample_kind, use_snconv=use_snconv),
                *(
                    gen_resblock.GeneratorResBlock(n_classes, in_channels[i], out_channels[i], 
        ...
        self.last_block = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Conv2d(out_channels[-1], 3, kernel_size=3, padding=1), 
        )
