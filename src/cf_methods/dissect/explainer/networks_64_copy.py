...
from src.cf_methods.dissect.explainer.ops_copy import (
    relu,
...
class Generator_Encoder_Decoder:
    ...
    def __call__(self, inputs, y, nums_class, num_channel=3):
        with tf.variable_scope(name_or_scope=self.name, reuse=tf.AUTO_REUSE):
            ...
            inputs = conv(
                "conv1", inputs, k_size=3, nums_out=64, strides=1
            )  # [n, 64, 64, 64]
            print(":", inputs)
            inputs = G_Resblock_Encoder(
                "Encoder-ResBlock3", inputs, 512, y, nums_class
            )  # [n, 32, 32, 512]
            print(":", inputs)
            inputs = G_Resblock_Encoder(
                "Encoder-ResBlock2", inputs, 1024, y, nums_class
            )  # [n, 16, 16, 1024]
            print(":", inputs)
            embedding = G_Resblock_Encoder(
                "Encoder-ResBlock1", inputs, 1024, y, nums_class
            )  # [n, 8, 8, 1024]
            print(":", embedding)

            inputs = G_Resblock(
                "ResBlock1", embedding, 1024, y, nums_class
            )  # [n, 16, 16, 1024]
            print(":", inputs)
            inputs = G_Resblock(
                "ResBlock2", inputs, 512, y, nums_class
            )  # [n, 32, 32, 512]
            print(":", inputs)
            inputs = G_Resblock(
                "ResBlock3", inputs, 256, y, nums_class
            )  # [n, 64, 64, 256]
            print(":", inputs)
            inputs = relu(conditional_batchnorm(inputs, "BN"))
            inputs = conv(
                "conv", inputs, k_size=3, nums_out=num_channel, strides=1
            )  # [n, 64, 64, 3]
            print(":", inputs)
...
class Discriminator_Ordinal:
    ...
    def __call__(self, inputs, y, nums_class, update_collection=None):
        with tf.variable_scope(name_or_scope=self.name, reuse=tf.AUTO_REUSE):
            # input: [n, 64, 64, 3]
            print(inputs)
            inputs = D_FirstResblock(
                "ResBlock1", inputs, 64, update_collection, is_down=True
            )  # [n, 32, 32, 64]
            print(inputs)
            inputs = D_Resblock(
                "ResBlock2", inputs, 128, update_collection, is_down=True
            )  # [n, 16, 16, 128]
            print(inputs)
            inputs = D_Resblock(
                "ResBlock3", inputs, 256, update_collection, is_down=True
            )  # [n, 8, 8, 256]
            print(inputs)
            inputs = D_Resblock(
                "ResBlock4", inputs, 512, update_collection, is_down=True
            )  # [n, 4, 4, 512]
            print(inputs)
            inputs = relu(inputs)
            ...
            for i in range(0, nums_class - 1):
                if i == 0:
                    temp = Inner_product(
                        inputs, y[:, i + 1], 2, update_collection
                    )  # [n, 1024]
                else:
                    temp = temp + Inner_product(
                        inputs, y[:, i + 1], 2, update_collection
                    )  # [n, 1024]
...
class Discriminator_Contrastive:
    ...
    def __call__(self, inputs, num_dims):
        with tf.variable_scope(name_or_scope=self.name, reuse=tf.AUTO_REUSE):
            ...
            print(inputs)
            inputs = D_FirstResblock(
                "ResBlock1", inputs, 64, None, is_down=True
            )  # [n, 32, 32, 64]
            print(inputs)
            inputs = D_Resblock(
                "ResBlock2", inputs, 128, None, is_down=True
            )  # [n, 16, 16, 128]
            print(inputs)
            inputs = relu(inputs)
...
class EncoderZ:
    ...
    def __call__(self, inputs, num_dims):
        with tf.variable_scope(name_or_scope=self.name, reuse=tf.AUTO_REUSE):
            ...
            print(":", inputs)
            inputs = Encoder_Block(
                "Encoder-ConvBlock2", inputs, 128
            )  # [n, 16, 16, 128]
            print(":", inputs)
            ...
            print(":", inputs)
            inputs = dense(
                "dense", inputs, 2 * num_dims
            )  # [n, 2*num_dims] 2 refers to mu and logvar
            inputs = relu(inputs)
            print(":", inputs)
            ...
            samples = tf.random_normal(
                shape=tf.shape(mu), mean=mu, stddev=tf.exp(0.5 * logvar)
            )
...
class EncoderW:
    ...
    def __call__(self, inputs, labels, num_dims):
        with tf.variable_scope(name_or_scope=self.name, reuse=tf.AUTO_REUSE):
            ...
            inputs = tf.concat(
                [inputs, tf.cast(tf.expand_dims(labels, -1), dtype=tf.float32)], axis=-1
            )  # [n, 257]
...
class DecoderX:
    ...
    def __call__(self, inputs):
        with tf.variable_scope(name_or_scope=self.name, reuse=tf.AUTO_REUSE):
            ...
            inputs = Decoder_Block(
                "Decoder-ConvBlock1", inputs, 256
            )  # [n, 16, 16, 256]
            print(":", inputs)
            inputs = Decoder_Block(
                "Decoder-ConvBlock2", inputs, 128
            )  # [n, 32, 32, 128]
            print(":", inputs)
