...
def spectral_normalization(
    name, weights, num_iters=1, update_collection=None, with_sigma=False
):
...
def conditional_batchnorm(x, scope_bn, y=None, nums_class=None):
    ...
    with tf.variable_scope(scope_bn):
        if y is None:
            ...
            beta, gamma = (
                tf.nn.embedding_lookup(beta, y),
                tf.nn.embedding_lookup(gamma, y),
            )
            beta = tf.reshape(beta, [-1, 1, 1, x.shape[-1]])
            gamma = tf.reshape(gamma, [-1, 1, 1, x.shape[-1]])
        batch_mean, batch_var = tf.nn.moments(
            x, [0, 1, 2], name="moments", keep_dims=True
        )
        ...
        mean, var = tf.cond(
            tf.less(tf.constant(2), tf.constant(5)),
...
def conv(name, inputs, nums_out, k_size, strides, update_collection=None, is_sn=False):
    ...
    with tf.variable_scope(name):
        W = tf.get_variable(
            "W",
            [k_size, k_size, nums_in, nums_out],
...
def dense(name, inputs, nums_out, update_collection=None, is_sn=False):
    ...
    with tf.variable_scope(name):
        W = tf.get_variable(
            "W", [nums_in, nums_out], initializer=tf.glorot_uniform_initializer()
        )
        b = tf.get_variable("b", [nums_out], initializer=tf.constant_initializer([0.0]))
        if is_sn:
            W = spectral_normalization("sn", W, update_collection=update_collection)
...
def Inner_product(global_pooled, y, nums_class, update_collection=None):
    ...
    V = tf.get_variable(
        "V", [nums_class, W], initializer=tf.glorot_uniform_initializer()
    )
    V = tf.transpose(V)
...
def G_Resblock(
    name, inputs, nums_out, y, nums_class, update_collection=None, is_sn=False
):
    with tf.variable_scope(name):
...
def D_Resblock(
    name, inputs, nums_out, update_collection=None, is_down=True, is_sn=True
):
    with tf.variable_scope(name):
        temp = tf.identity(inputs)
        inputs = relu(inputs)
        ...
        if is_down:
            ...
            # Identity mapping
            temp = conv(
                "identity", temp, nums_out, 1, 1, update_collection, is_sn=is_sn
            )  # replacing identity mapping with 1x1 conv
...
def D_FirstResblock(
    name, inputs, nums_out, update_collection, is_down=True, is_sn=True
):
    with tf.variable_scope(name):
        temp = tf.identity(inputs)
        inputs = conv(
...
def KL(mu1, logvar1, mu2, logvar2):
    ...
    return tf.reduce_sum(
        safe_log(std2)
        - safe_log(std1)
        + 0.5 * (tf.exp(logvar1) + (mu1 - mu2) ** 2) / tf.exp(logvar2)
        - 0.5,
        axis=-1,
    )
