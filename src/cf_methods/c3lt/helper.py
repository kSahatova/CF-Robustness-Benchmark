...
def classifier_loss(classifier, images, target_class, args, reduction="mean"):
    ...
    if args.cls_type == "hinge":
        ...
        loss = torch.maximum(
            -output_target + max_output_non_target + kappa,
            torch.zeros(
                num_samples,
            ).to(images.device),
        )
...
def smoothness_loss(masks, beta=2, reduction="mean"):
    ...
    masks = masks[:, 0, :, :]
    a = torch.mean(
        torch.abs((masks[:, :-1, :] - masks[:, 1:, :]).view(masks.shape[0], -1)).pow(
            beta
        ),
        dim=1,
    )
...
def save_tensors(tensors, args, index="", extra_inf="", preds=None):
    ...
    if preds is None:
        save_image(
            torch.cat(tensors, 0),
            "{}/{}_{}_{}.png".format(args.output, index, args.method, extra_inf),
            nrow=args.sample_num,
            normalize=True,
            padding=0,
        )
    else:
...
def save_as_gif(tensors, args, index, folder_name):
    ...
    save_image(
        torch.cat(tensors, 0),
        "{}/{}.png".format(out_dir, index),
        nrow=2 * args.sample_num,
        normalize=True,
    )
